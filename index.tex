...
\textbf{Week 1-6 Tuesday @ Central Teaching Hub PCTC}
...
\textbf{Deadline}: Thursday 12th November 2026 - 2pm. \textbf{Word
count}: 2000 words - including tables, excluding references.
...
Normally you don't need to do so, but if something does not work, you
may have to install \href{quarto.org}{Quarto}. Recent versions of
RStudio do not require you to do so. installed.

\section*{.html file}\label{html-file}
\addcontentsline{toc}{section}{.html file}

\markright{.html file}

You can submit a \texttt{.html} file, that is a rendered version of a
...
To do so, at the top of your document, include something like this in a
YAML cell (see
\href{https://theuniversityofliverpool-my.sharepoint.com/:u:/r/personal/gfilo_liverpool_ac_uk/Documents/Teaching/stats/general/reportTemplate.qmd?csf=1&web=1&e=iPjZoT}{Template},
in RStudio, ``visual view'', \texttt{Insert}, \texttt{YAML\ block})
...
\tightlist
\item
  Click the \textbf{Render} button in the RStudio toolbar (blue arrow).
\end{enumerate}
...
Updated upstream \textbf{Suggested Report Structure} --- This template
follows the requested sections. Each section contains guidance text and
an empty R code cell for your analysis. Download it from
\href{https://theuniversityofliverpool-my.sharepoint.com/:u:/g/personal/gfilo_liverpool_ac_uk/EXgnQW7AoaFFjo7iOknuNbQBjsOuZtU-RcwtK5tUZB-97Q}{here}
(you need to be logged in with the UoL account) and work on it with your
own Rstudio. ======= \textbf{Suggested Report Structure} --- This
template follows the requested sections. Each section contains guidance
text and an empty R code cell for your analysis. Download it from
...
\chapter{Lab: Data Visualisation with
ggplot}\label{lab-data-visualisation-with-ggplot}

The lecture's slides can be found
\href{https://github.com/GDSL-UL/stats/blob/main/lectures/lecture5.pdf}{here}.

\section{Part 1: Towards the Assignment (30 min, or till when you feel
you are good to
go)}\label{part-1-towards-the-assignment-30-min-or-till-when-you-feel-you-are-good-to-go}
...
\section{\texorpdfstring{Part 2 - Visualisation: \texttt{ggplot2}
Functions and
Arguments}{Part 2 - Visualisation: ggplot2 Functions and Arguments}}\label{part-2---visualisation-ggplot2-functions-and-arguments}

For this session you need the following libraries:

\texttt{install.packages(c("ggplot2",\ "dplyr",\ "tidyr",\ "kableExtra",\ "ggridges",\ "RColorBrewer",\ "broom",\ "scales",\ "corrplot",\ "vtable"))}

If necessary install them by moving the line into a code chunk.

The \textbf{\texttt{ggplot2}} package in R is one of the most powerful
tools for creating publication-quality visualisations. It uses a layered
approach to building plots, starting with data, then adding mappings,
geometries, and other components.

A \texttt{ggplot2} plot is built step by step:
...
\subsection{\texorpdfstring{\textbf{\texttt{ggplot()}}}{ggplot()}}\label{ggplot}
...
\item
  Main arguments:
\item
  \texttt{data}: A data frame containing the variables to be plotted.
\item
  \texttt{aes()}: Aesthetic mappings to connect data variables to visual
  properties like \texttt{x}, \texttt{y}, \texttt{color}, \texttt{fill},
  \texttt{size}, etc.
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{census\_data }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"../data/Census2021/EW\_DistrictPercentages.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(}\AttributeTok{data =}\NormalTok{ census\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Residents, }\AttributeTok{y =}\NormalTok{ mean\_age)) }\SpecialCharTok{+} 
 \FunctionTok{geom\_point}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-3-1.pdf}}

\subsection{Geometries in ggplot2}\label{geometries-in-ggplot2}
...
\bottomrule\noalign{}
\endlastfoot
Point & \texttt{geom\_point()} & Scatterplots \\
Line & \texttt{geom\_line()} & Line plots \\
Bar & \texttt{geom\_bar()} & Bar charts \\
Histogram & \texttt{geom\_histogram()} & Histograms \\
Boxplot & \texttt{geom\_boxplot()} & Boxplots \\
Density & \texttt{geom\_density()} & Density plots \\
Smooth & \texttt{geom\_smooth()} & Add regression or smoothing lines \\
\end{longtable}
}

\textbf{Example: Scatterplot with \texttt{geom\_point()}}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(census\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Residents, }\AttributeTok{y =}\NormalTok{ mean\_age, }\AttributeTok{color =} \FunctionTok{factor}\NormalTok{(Region))) }\SpecialCharTok{+}
 \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{size =} \DecValTok{3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-4-1.pdf}}

\subsection{\texorpdfstring{Aesthetics:
\texttt{aes()}}{Aesthetics: aes()}}\label{aesthetics-aes}
...
\item
  Common aesthetics:
\item
  \texttt{x}: X-axis variable.
\item
  \texttt{y}: Y-axis variable.
\item
  \texttt{color}: Changes point/line colors based on a variable.
\item
  \texttt{fill}: Fills bars/areas based on a variable.
\item
  \texttt{size}: Controls the size of points/lines.
\end{itemize}

\textbf{Example: Adding color and size aesthetics}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(census\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Residents, }\AttributeTok{y =}\NormalTok{ mean\_age, }\AttributeTok{color =} \FunctionTok{factor}\NormalTok{(Region), }\AttributeTok{size =}\NormalTok{ pct\_Very\_bad\_health)) }\SpecialCharTok{+}
 \FunctionTok{geom\_point}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-5-1.pdf}}

What's missing in the plot above?How can this be improved?

\subsection{\texorpdfstring{Faceting:
\texttt{facet\_*}}{Faceting: facet\_*}}\label{faceting-facet_}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{facet\_wrap(\textasciitilde{}var)} & Wraps plots across
rows/columns. \\
\texttt{facet\_grid(row\ \textasciitilde{}\ col)} & Creates a grid
layout for plots. \\
\end{longtable}
}

Faceting in is a technique to create multiple plots based on subsets of
your data. It is particularly useful for comparing relationships or
distributions across categories. By breaking down a dataset into smaller
pieces according to a categorical variable, faceting helps visualise
data trends or differences within subgroups. For instance,
\texttt{facet\_wrap(\textasciitilde{}\ Region)} creates individual plots
for each category in the Region variable, aligning them into rows and
columns. Alternatively,
\texttt{facet\_grid(row\ \textasciitilde{}\ col)} creates a more
structured layout, where two categorical variables determine the rows
and columns of the grid.

Faceting is essential when dealing with data that needs to be compared
across multiple dimensions. For example, a scatterplot faceted by sex
can show how a relationship differs for males and females. Similarly,
faceted histograms can reveal differences in the distribution of a
variable across categories.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(census\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Residents, }\AttributeTok{y =}\NormalTok{ mean\_age)) }\SpecialCharTok{+}
 \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
 \FunctionTok{facet\_wrap}\NormalTok{(}\SpecialCharTok{\textasciitilde{}}\NormalTok{Region)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-6-1.pdf}}

What are the issues here?

\subsection{\texorpdfstring{Themes
(\texttt{theme\_*})}{Themes (theme\_*)}}\label{themes-theme_}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{theme\_minimal()} & Simple and clean theme. \\
\texttt{theme\_classic()} & Classic-style plots. \\
\texttt{theme\_dark()} & Dark background. \\
\texttt{theme\_void()} & Minimal with no axes or grids. \\
\end{longtable}
}

\textbf{Applying a theme}
...
\FunctionTok{ggplot}\NormalTok{(}\AttributeTok{data =}\NormalTok{ census\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Residents, }\AttributeTok{y =}\NormalTok{ mean\_age)) }\SpecialCharTok{+} 
 ...
 \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Visualisation with Void Theme"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-7-1.pdf}}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{scale\_color\_manual()} & Customizes colors for lines/points. \\
\texttt{scale\_fill\_brewer()} & Predefined color palettes for fills. \\
\texttt{scale\_x\_continuous()} & Modifies X-axis properties. \\
\texttt{scale\_y\_continuous()} & Modifies Y-axis properties. \\
\end{longtable}
}

\texttt{scale\_color} functions modify the outline color of elements
like points, lines, or the border of shapes, while \texttt{scale\_fill}
functions modify the interior fill color of shapes like bars, tiles, or
boxes. Both are used to control aesthetics based on a variable mapped in
\texttt{aes()} but apply to different visual aspects of the plot.

\textbf{Example: Customizing axis and colors}

The \texttt{RColorBrewer} library provides pre-defined color palettes
specifically designed for data visualisation, ensuring clarity and
accessibility. It includes sequential, diverging, and qualitative
palettes suitable for various data types and visualisation needs.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Automatically determine the number of colors needed}

\NormalTok{num\_levels }\OtherTok{\textless{}{-}} \FunctionTok{length}\NormalTok{(}\FunctionTok{unique}\NormalTok{(census\_data}\SpecialCharTok{$}\NormalTok{Region))}
\end{Highlighting}
\end{Shaded}

The \texttt{num\_levels} variable is a dynamic way to determine the
number of unique levels in a categorical variable, such as
\texttt{Region}. This approach is particularly helpful when you don't
know in advance how many categories exist in the dataset.

The code above calculates the number of unique categories in the
\texttt{Region} variable, ensuring that your plot's color palette
matches the data's requirements. Based on the value of
\texttt{num\_levels}, you can choose an appropriate color palette from
libraries like \texttt{RColorBrewer}. For example:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(RColorBrewer)}
\CommentTok{\# Use an appropriate Brewer palette based on the number of levels}
\NormalTok{palette }\OtherTok{\textless{}{-}} \ControlFlowTok{if}\NormalTok{ (num\_levels }\SpecialCharTok{\textless{}=} \DecValTok{8}\NormalTok{) }\StringTok{"Set2"} \ControlFlowTok{else} \StringTok{"Paired"} \CommentTok{\# Choose a palette with enough colors}
\end{Highlighting}
\end{Shaded}

This dynamically selects a palette suitable for the number of
categories. If there are 8 or fewer levels, the \texttt{Set2} palette is
used; for more levels, \texttt{Paired} ensures enough distinct colors
(there's several palettes, check the link above for more).

When applying the palette in \texttt{ggplot2}, the
function\texttt{scale\_color\_brewer(palette\ =\ palette)} ensures that
the plot assigns colors based on the selected scheme. This process
avoids hardcoding colors or palettes and ensures the plot adapts to
changes in the data, such as when categories are added or removed.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(census\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Residents, }\AttributeTok{y =}\NormalTok{ mean\_age, }\AttributeTok{color =} \FunctionTok{factor}\NormalTok{(Region))) }\SpecialCharTok{+}
 \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{size =} \DecValTok{3}\NormalTok{) }\SpecialCharTok{+}
 \FunctionTok{scale\_color\_brewer}\NormalTok{(}\AttributeTok{palette =}\NormalTok{ palette) }\SpecialCharTok{+} \CommentTok{\# Dynamically chosen palette}
 ...
 \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{" Write a title"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-10-1.pdf}}
...
\FunctionTok{ggplot}\NormalTok{(census\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Residents, }\AttributeTok{y =}\NormalTok{ mean\_age, }\AttributeTok{color =} \FunctionTok{factor}\NormalTok{(Region))) }\SpecialCharTok{+}
 \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{size =} \DecValTok{3}\NormalTok{) }\SpecialCharTok{+}
 \FunctionTok{scale\_color\_manual}\NormalTok{(}\AttributeTok{values =} \FunctionTok{c}\NormalTok{(}\StringTok{"red"}\NormalTok{, }\StringTok{"blue"}\NormalTok{, }\StringTok{"green"}\NormalTok{, }\StringTok{"purple"}\NormalTok{, }\StringTok{"orange"}\NormalTok{, }\StringTok{"pink"}\NormalTok{, }\StringTok{"brown"}\NormalTok{, }\StringTok{"yellow"}\NormalTok{, }\StringTok{"gray"}\NormalTok{, }\StringTok{"skyblue"}\NormalTok{)) }\SpecialCharTok{+}
 \FunctionTok{labs}\NormalTok{(}
  \AttributeTok{title =} \StringTok{"Scatterplot of Residents vs Mean Age by Region"}\NormalTok{,}
  \AttributeTok{x =} \StringTok{"Residents"}\NormalTok{,}
  \AttributeTok{y =} \StringTok{"Mean Age"}\NormalTok{,}
...
\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-11-1.pdf}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Discrete Color Scales} & \texttt{scale\_color\_brewer()} & Use
pre-defined discrete palettes from RColorBrewer. &
\texttt{scale\_color\_brewer(palette\ =\ "Set1")} \\
& \texttt{scale\_fill\_brewer()} & Fill colors for discrete variables
from RColorBrewer. &
\texttt{scale\_fill\_brewer(palette\ =\ "Pastel2")} \\
& \texttt{scale\_color\_manual()} & Manually assign colors for discrete
variables. &
\texttt{scale\_color\_manual(values\ =\ c("red",\ "blue",\ "green"))} \\
& \texttt{scale\_fill\_manual()} & Manually assign fill colors for
discrete variables. &
\texttt{scale\_fill\_manual(values\ =\ c("purple",\ "orange",\ "yellow"))} \\
& \texttt{scale\_color\_viridis\_d()} & Use discrete palettes from the
viridis package (colorblind-friendly). &
\texttt{scale\_color\_viridis\_d(option\ =\ "plasma")} \\
& \texttt{scale\_fill\_viridis\_d()} & Fill discrete variables with
viridis palettes. &
\texttt{scale\_fill\_viridis\_d(option\ =\ "cividis")} \\
\textbf{Continuous Color Scales} & \texttt{scale\_color\_gradient()} &
Two-color gradient for continuous variables. &
\texttt{scale\_color\_gradient(low\ =\ "blue",\ high\ =\ "red")} \\
& \texttt{scale\_fill\_gradient()} & Two-color gradient for continuous
fill variables. &
\texttt{scale\_fill\_gradient(low\ =\ "green",\ high\ =\ "yellow")} \\
& \texttt{scale\_color\_gradient2()} & Diverging gradient with a
midpoint for continuous variables. &
\texttt{scale\_color\_gradient2(low\ =\ "blue",\ mid\ =\ "white",\ high\ =\ "red",\ midpoint\ =\ 0)} \\
& \texttt{scale\_fill\_gradient2()} & Diverging gradient with a midpoint
for continuous fill variables. &
\texttt{scale\_fill\_gradient2(low\ =\ "purple",\ mid\ =\ "gray",\ high\ =\ "orange",\ midpoint\ =\ 50)} \\
& \texttt{scale\_color\_gradientn()} & Multi-color gradient for
continuous variables. &
\texttt{scale\_color\_gradientn(colors\ =\ c("blue",\ "green",\ "yellow",\ "red"))} \\
& \texttt{scale\_fill\_gradientn()} & Multi-color gradient for
continuous fill variables. &
\texttt{scale\_fill\_gradientn(colors\ =\ c("lightblue",\ "white",\ "pink"))} \\
& \texttt{scale\_color\_viridis\_c()} & Continuous color scales from
viridis. & \texttt{scale\_color\_viridis\_c(option\ =\ "magma")} \\
& \texttt{scale\_fill\_viridis\_c()} & Continuous fill scales from
viridis. & \texttt{scale\_fill\_viridis\_c(option\ =\ "inferno")} \\
\textbf{Axis Scales} & \texttt{scale\_x\_continuous()} & Customize
numeric X-axis with limits and breaks. &
\texttt{scale\_x\_continuous(limits\ =\ c(0,\ 100),\ breaks\ =\ seq(0,\ 100,\ 10))} \\
& \texttt{scale\_y\_continuous()} & Customize numeric Y-axis with limits
and labels. &
\texttt{scale\_y\_continuous(labels\ =\ scales::percent\_format())} \\
& \texttt{scale\_x\_log10()} & Logarithmic transformation for X-axis. &
\texttt{scale\_x\_log10()} \\
& \texttt{scale\_y\_log10()} & Logarithmic transformation for Y-axis. &
\texttt{scale\_y\_log10()} \\
& \texttt{scale\_x\_reverse()} & Reverse the X-axis direction. &
\texttt{scale\_x\_reverse()} \\
& \texttt{scale\_y\_reverse()} & Reverse the Y-axis direction. &
\texttt{scale\_y\_reverse()} \\
\textbf{Shape and Size Scales} & \texttt{scale\_shape\_manual()} &
Manually assign shapes to categories for points. &
\texttt{scale\_shape\_manual(values\ =\ c(19,\ 17,\ 15))} \\
& \texttt{scale\_size\_continuous()} & Scale the size of points based on
a continuous variable. &
\texttt{scale\_size\_continuous(range\ =\ c(1,\ 10))} \\
& \texttt{scale\_size\_manual()} & Manually specify point sizes for
categorical variables. &
\texttt{scale\_size\_manual(values\ =\ c(2,\ 4,\ 6))} \\
\end{longtable}
}

\subsection{Additional Functions for
Customization}\label{additional-functions-for-customization}

\begin{itemize}
\tightlist
\item
  \textbf{\texttt{labs()}}: Adds labels for axes, title, and legend.
\item
  \textbf{\texttt{coord\_flip()}}: Flips X and Y axes for horizontal
  plots.
\end{itemize}
...
\FunctionTok{labs}\NormalTok{(}
 ...
 \AttributeTok{y =} \StringTok{"Y{-}Axis Label"}\NormalTok{,}
 \AttributeTok{color =} \StringTok{"Legend Title"}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\# flipping}
\FunctionTok{ggplot}\NormalTok{(census\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =} \FunctionTok{factor}\NormalTok{(Region), }\AttributeTok{y =}\NormalTok{ Residents)) }\SpecialCharTok{+}
 \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+}
 \FunctionTok{coord\_flip}\NormalTok{() }\SpecialCharTok{+}
 \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Horizontal Boxplot"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-13-1.pdf}}

\textbf{Key Tips for \texttt{ggplot2}}:

\begin{itemize}
\tightlist
\item
  Start with simple plots and incrementally add layers (\texttt{+}).
\item
  Use themes (\texttt{theme\_minimal()}, \texttt{theme\_classic()}) to
  clean up your plots.
\item
  Explore palettes with \texttt{RColorBrewer} or \texttt{viridis} for
  colorblind-friendly options.
\end{itemize}

\section{Part 3 - Visualisation: Making decent graphs
(1h)}\label{part-3---visualisation-making-decent-graphs-1h}

This section demonstrates how to create \textbf{publication-quality
visualisations} in R using \texttt{ggplot2}.

\textbf{Learning goals} Developing an understanding of, and ability to
create academic standard data visualisations.

\textbf{Let's load the datasets first} Check the paths, you may need to
remove the ``..''
...
\subsection{Distribution of 1 Numerical
variable:}\label{distribution-of-1-numerical-variable}

\textbf{Histogram}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(census\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ pct\_Age\_20\_to\_29)) }\SpecialCharTok{+} 
 \FunctionTok{geom\_histogram}\NormalTok{(}\AttributeTok{bins =} \DecValTok{5}\NormalTok{, }\AttributeTok{fill =} \StringTok{"steelblue"}\NormalTok{, }\AttributeTok{color =} \StringTok{"black"}\NormalTok{) }\SpecialCharTok{+}
 \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Distribution of Numerical Variable"}\NormalTok{, }\AttributeTok{x =} \StringTok{"Variable"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Count"}\NormalTok{) }\SpecialCharTok{+}
 \FunctionTok{theme\_minimal}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-15-1.pdf}}

The \texttt{bins} parameter in a histogram determines the number of
intervals (bins) into which the data range is divided. Each bin
represents a range of values along the x-axis, and the height of each
bar shows the count of observations within that range. For example, if
bins = 5, the data is divided into five equal-width intervals. Using
fewer bins results in a coarser view of the data, while more bins
provide a finer, more detailed representation. However, too many bins
can make the histogram appear cluttered and difficult to interpret. The
width of each bin is automatically calculated by dividing the range of
the data by the number of bins.

\textbf{Density Plot}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(census\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Residents)) }\SpecialCharTok{+} 
 ...
 \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Residents, Density distribution"}\NormalTok{, }\AttributeTok{x =} \StringTok{"Value"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Density"}\NormalTok{) }\SpecialCharTok{+}
 \FunctionTok{theme\_minimal}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-16-1.pdf}}

\textbf{Histogram + Density Distribution}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Plot histogram with density overlay for a chosen variable (e.g., \textquotesingle{}pct\_No\_qualifications\textquotesingle{})}
\FunctionTok{ggplot}\NormalTok{(census\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ pct\_No\_qualifications)) }\SpecialCharTok{+} 
    \FunctionTok{geom\_histogram}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =} \FunctionTok{after\_stat}\NormalTok{(density)), }\AttributeTok{bins =} \DecValTok{10}\NormalTok{, }\AttributeTok{color =} \StringTok{"black"}\NormalTok{, }\AttributeTok{fill =} \StringTok{"skyblue"}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.7}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{geom\_density}\NormalTok{(}\AttributeTok{color =} \StringTok{"darkblue"}\NormalTok{, }\AttributeTok{linewidth =} \DecValTok{1}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Distribution of pct\_No\_qualifications"}\NormalTok{, }\AttributeTok{x =} \StringTok{"Value"}\NormalTok{, }\AttributeTok{y =}  \StringTok{"Density"}\NormalTok{) }\SpecialCharTok{+}
 \FunctionTok{theme\_minimal}\NormalTok{()}
\end{Highlighting}
...
\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-17-1.pdf}}

\textbf{Box Plot}

A box plot, also known as a box-and-whisker plot, is a graphical
representation of the distribution of a dataset. It displays the
minimum, first quartile (Q1), median (Q2), third quartile (Q3), and
maximum values, with the central box representing the interquartile
range (IQR) from Q1 to Q3. The whiskers extend from the box to show the
range of the data, excluding outliers. Outliers are typically
represented as individual points outside the whiskers.

The box plot provides a concise summary of the data distribution,
highlighting the spread, center, and potential skewness.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(scales) }\CommentTok{\# For label\_comma()}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(census\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ Residents)) }\SpecialCharTok{+} 
 ...
 \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Residents Distribution"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Number of Residents"}\NormalTok{) }\SpecialCharTok{+} 
 \FunctionTok{theme\_minimal}\NormalTok{() }\SpecialCharTok{+}
 \FunctionTok{scale\_y\_continuous}\NormalTok{(}
  \AttributeTok{limits =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{500000}\NormalTok{), }
  \AttributeTok{labels =} \FunctionTok{label\_comma}\NormalTok{() }\CommentTok{\# Properly placed within scale\_y\_continuous()}
\NormalTok{ )}
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-18-1.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Setting axis limits for better readability and applying comma{-}separated format to large numbers.}
\end{Highlighting}
\end{Shaded}

This looks kind of sad with just one variable, but it's the best way,
usually, to plot distributions for numerical variables.

Notice the \textbf{outliers}. Outliers are data points that deviate
significantly from the overall pattern of a dataset. They can arise due
to measurement errors, variability in the data, or the presence of
extreme values. Outliers can heavily influence statistical analyses,
such as means or regression models, potentially leading to misleading
conclusions. Identifying and addressing outliers is crucial to ensure
robust and accurate results. Boxplots allow spotting them, along with
interquartile range (IQR) analysis.

\subsection{Distribution of 1 Categorical
variable:}\label{distribution-of-1-categorical-variable}
...
\FunctionTok{ggplot}\NormalTok{(frs\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ highest\_qual)) }\SpecialCharTok{+} 
 ...
 \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Distribution of 1 Categorical Variable"}\NormalTok{, }\AttributeTok{x =} \StringTok{"Highest Educational Qualification"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Count"}\NormalTok{) }\SpecialCharTok{+}
 \FunctionTok{theme\_minimal}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-19-1.pdf}}

\textbf{Horizontal Bar Chart}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(frs\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ highest\_qual)) }\SpecialCharTok{+} 
 \FunctionTok{geom\_bar}\NormalTok{(}\AttributeTok{fill =} \StringTok{"lightgreen"}\NormalTok{, }\AttributeTok{color =} \StringTok{"black"}\NormalTok{) }\SpecialCharTok{+}
 \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Horizontal Bar Chart"}\NormalTok{, }\AttributeTok{x =} \StringTok{"Highest Educational Qualification"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Count"}\NormalTok{, ) }\SpecialCharTok{+}
 \FunctionTok{coord\_flip}\NormalTok{() }\SpecialCharTok{+} \CommentTok{\#just flipping}
 \FunctionTok{theme\_minimal}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-20-1.pdf}}

\subsection{Comparing variables}\label{comparing-variables}

\textbf{1 numerical, 1 categorical: Boxplot}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(frs\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ highest\_qual, }\AttributeTok{y =}\NormalTok{ hh\_size, }\AttributeTok{fill =}\NormalTok{ highest\_qual)) }\SpecialCharTok{+}
 \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+}
 \FunctionTok{labs}\NormalTok{(}
  \AttributeTok{title =} \StringTok{"Household Size by Highest Educational Qualification"}\NormalTok{,}
  \AttributeTok{x =} \StringTok{"Highest Educational Qualification"}\NormalTok{,}
  \AttributeTok{y =} \StringTok{"Household Size"}
\NormalTok{ ) }\SpecialCharTok{+}
 \FunctionTok{theme\_minimal}\NormalTok{() }\SpecialCharTok{+}
 \FunctionTok{theme}\NormalTok{(}\AttributeTok{legend.position =} \StringTok{"none"}\NormalTok{) }\CommentTok{\# don\textquotesingle{}t need this}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-21-1.pdf}}

\textbf{1 numerical, 1 categorical: Density Ridges}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(ggridges) }\CommentTok{\# need a library for this}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(frs\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ hh\_size, }\AttributeTok{y =}\NormalTok{ highest\_qual, }\AttributeTok{fill =}\NormalTok{ highest\_qual)) }\SpecialCharTok{+}
 \FunctionTok{geom\_density\_ridges}\NormalTok{(}\AttributeTok{alpha =} \FloatTok{0.7}\NormalTok{) }\SpecialCharTok{+}
 \FunctionTok{labs}\NormalTok{(}
  \AttributeTok{title =} \StringTok{"Household Size Distribution by Highest Qualification"}\NormalTok{,}
  \AttributeTok{x =} \StringTok{"Household Size"}\NormalTok{,}
  \AttributeTok{y =} \StringTok{"Highest Qualification"}
\NormalTok{ ) }\SpecialCharTok{+}
 \FunctionTok{theme\_minimal}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-22-1.pdf}}

\textbf{2 Categorical Variables: Stacked Bar Chart}

Note the adjustments made for making the labels look better

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(frs\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ highest\_qual, }\AttributeTok{fill =}\NormalTok{ health)) }\SpecialCharTok{+}
 \FunctionTok{geom\_bar}\NormalTok{(}\AttributeTok{position =} \StringTok{"stack"}\NormalTok{) }\SpecialCharTok{+}
 \FunctionTok{labs}\NormalTok{(}
  \AttributeTok{title =} \StringTok{"Health Status by Highest Qualification"}\NormalTok{,}
  \AttributeTok{x =} \StringTok{"Highest Qualification"}\NormalTok{,}
  \AttributeTok{y =} \StringTok{"Count of Individuals"}\NormalTok{,}
  \AttributeTok{fill =} \StringTok{"Health Status"}
\NormalTok{ ) }\SpecialCharTok{+}
 \FunctionTok{theme\_minimal}\NormalTok{() }\SpecialCharTok{+}
 \FunctionTok{theme}\NormalTok{(}
  \AttributeTok{axis.text.x =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{angle =} \DecValTok{45}\NormalTok{, }\AttributeTok{hjust =} \DecValTok{1}\NormalTok{), }\CommentTok{\# Rotate X{-}axis labels for readability}
  \AttributeTok{legend.title =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{size =} \DecValTok{12}\NormalTok{),      }\CommentTok{\# Adjust legend title size}
  \AttributeTok{legend.text =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{size =} \DecValTok{10}\NormalTok{)       }\CommentTok{\# Adjust legend text size}
\NormalTok{ )}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-23-1.pdf}}

\textbf{Side-by-Side Bar Chart (Two-Way Frequency Distribution)}

This type of chart can get messy.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(frs\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ health, }\AttributeTok{fill =}\NormalTok{ highest\_qual)) }\SpecialCharTok{+}
...
\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-24-1.pdf}}

\textbf{2 Categorical Variable counts to percentages: Stacked Percentage
Graph}

One can also convert the counts of two categorical variables to
percentages. In this case, we aim to visualise the proportional
distribution of one categorical variable (\texttt{health}) across levels
of another categorical variable (\texttt{highest\_qual}) using a stacked
percentage bar chart. First, we create a cross-tabulation to count the
occurrences of each combination of Health and Qualification categories.
These counts are then converted into a percentage format relative to the
total counts for each Qualification category. This transformation allows
us to represent the relative proportions rather than absolute counts,
facilitating comparison across categories.

A \emph{cross tabulation} (or contingency table) is a statistical tool
used to analyze the relationship between two or more categorical
variables. It organizes data into a table format where: - Rows represent
the categories of one variable. - Columns represent the categories of
another variable. - Cells display the counts or frequencies of data
points that fall into each combination of the row and column categories.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Create a cross{-}tabulation of observed counts }
\NormalTok{cross\_tab }\OtherTok{\textless{}{-}} \FunctionTok{table}\NormalTok{(frs\_data}\SpecialCharTok{$}\NormalTok{health, frs\_data}\SpecialCharTok{$}\NormalTok{highest\_qual) }

\CommentTok{\# Convert the cross{-}tabulation to a data frame}
\NormalTok{cross\_tab\_df }\OtherTok{\textless{}{-}} \FunctionTok{as.data.frame}\NormalTok{(cross\_tab)}
\FunctionTok{colnames}\NormalTok{(cross\_tab\_df) }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"Health"}\NormalTok{, }\StringTok{"Qualification"}\NormalTok{, }\StringTok{"Percentage"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

The data is reshaped into a data frame for compatibility with
\texttt{ggplot2}, where a stacked bar chart with percentage scaling
\texttt{(position\ =\ "fill")} is generated. The result is a plot that
shows how Health statuses are distributed proportionally within each
Qualification level.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Create a stacked percentage bar chart}
\FunctionTok{ggplot}\NormalTok{(cross\_tab\_df, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Qualification, }\AttributeTok{y =}\NormalTok{ Percentage, }\AttributeTok{fill =}\NormalTok{ Health)) }\SpecialCharTok{+}
 ...
 \FunctionTok{labs}\NormalTok{(}
  \AttributeTok{title =} \StringTok{"Health by Highest Qualification (Percentage)"}\NormalTok{,}
  \AttributeTok{x =} \StringTok{"Highest Qualification"}\NormalTok{,}
...
\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-26-1.pdf}}

\textbf{3+ Numerical Variables: Boxplot}

To compare three numerical variables using boxplots in ggplot2, one
needs to reshape the data to a long format so that each numerical
variable is treated as a category in a single column.

Here's how you can do it, using \texttt{tidyverse}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(tidyverse) }\CommentTok{\# we need tidyverse for this}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{long\_data }\OtherTok{\textless{}{-}}\NormalTok{ census\_data }\SpecialCharTok{\%\textgreater{}\%}
 \FunctionTok{pivot\_longer}\NormalTok{(}
  \AttributeTok{cols =} \FunctionTok{c}\NormalTok{(pct\_Single, pct\_Muslim, pct\_Married\_opposite\_sex\_couple),}
...
The code above reshapes the data from a wide format to a long format
using the \texttt{pivot\_longer()} function from the \texttt{tidyverse}
package. In the original dataset, each column represents a separate
variable (e.g., \texttt{pct\_Single}, \texttt{pct\_Muslim}, ..), and
their values are stored in individual columns. The transformation
collapses these columns into two new columns: one for the variable names
(Variable) and another for their corresponding values (Value). This
format is useful for plotting or analysis where variables are treated
uniformly, such as when creating boxplots or facet grids in ggplot2.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Create the boxplot}
\FunctionTok{ggplot}\NormalTok{(long\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Variable, }\AttributeTok{y =}\NormalTok{ Value, }\AttributeTok{fill =}\NormalTok{ Variable)) }\SpecialCharTok{+}
 \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+}
 \FunctionTok{labs}\NormalTok{(}
  \AttributeTok{title =} \StringTok{"Comparison of Three Numerical Variables"}\NormalTok{,}
...
\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-28-1.pdf}}

\subsection{Visualising Relationships:}\label{visualising-relationships}

\textbf{2 Numerical Variables: Scatterplot}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(census\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ pct\_Single, }\AttributeTok{y =}\NormalTok{ pct\_Muslim, }\AttributeTok{color =} \FunctionTok{factor}\NormalTok{(Country))) }\SpecialCharTok{+}
 ...
 \FunctionTok{labs}\NormalTok{(}
  \AttributeTok{title =} \StringTok{"Percentage of Single Population vs Muslim Population by Country"}\NormalTok{,}
  \AttributeTok{x =} \StringTok{"Percentage of Single Population"}\NormalTok{,}
  \AttributeTok{y =} \StringTok{"Percentage of Muslim Population"}\NormalTok{,}
...
\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-29-1.pdf}}

\textbf{2 Numerical Variables + 1 Catecorical: Scatterplot}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(census\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ pct\_Single, }\AttributeTok{y =}\NormalTok{ pct\_Muslim, }\AttributeTok{color =} \FunctionTok{factor}\NormalTok{(Region))) }\SpecialCharTok{+}
 \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{size =} \DecValTok{3}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.7}\NormalTok{) }\SpecialCharTok{+} 
 \FunctionTok{labs}\NormalTok{(}
  \AttributeTok{title =} \StringTok{"Percentage of Single Population vs Muslim Population by Region"}\NormalTok{,}
  \AttributeTok{x =} \StringTok{"Percentage of Single Population"}\NormalTok{,}
  \AttributeTok{y =} \StringTok{"Percentage of Muslim Population"}\NormalTok{,}
  \AttributeTok{color =} \StringTok{"Region"}
\NormalTok{ ) }\SpecialCharTok{+}
...
\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-30-1.pdf}}

\textbf{3+ Numerical Variables: Correlogram}

A correlogram is a visual representation of a correlation matrix, where
the strength and direction of relationships between numerical variables
are displayed. The matrix is typically a grid, with variables listed
along both the rows and columns. Each cell in the matrix shows the
correlation coefficient between two variables.

The library \texttt{corrplot} would do it for us. Check:
https://cran.r-project.org/web/packages/corrplot/vignettes/corrplot-intro.html
for deeper customisation.

In the corrplot package, a correlogram can use colors, shapes, or
numbers to represent the correlation. For example, colors ranging from
blue to red often signify negative to positive correlations, while white
may indicate no correlation. Similarly, circles or other shapes can vary
in size to depict the strength of the relationship. The corrplot
function simplifies creating correlograms with customizable options. For
example, using method = ``number'' places correlation coefficients in
each cell, while method = ``circle'' represents correlations
graphically.

Correlograms are valuable for quickly identifying patterns or strong
relationships between variables, such as spotting which features might
influence one another in a dataset
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Select the necessary variables from the dataset}
\NormalTok{selected\_vars }\OtherTok{\textless{}{-}}\NormalTok{ census\_data[, }\FunctionTok{c}\NormalTok{(}\StringTok{"pct\_Males"}\NormalTok{, }\StringTok{"pct\_Age\_85\_and\_over"}\NormalTok{, }\StringTok{"pct\_Bad\_health"}\NormalTok{)]}

\CommentTok{\# Calculate the correlation matrix}
\NormalTok{cor\_matrix }\OtherTok{\textless{}{-}} \FunctionTok{cor}\NormalTok{(selected\_vars, }\AttributeTok{use =} \StringTok{"complete.obs"}\NormalTok{)}

\CommentTok{\# Generate the correlation plot}
\FunctionTok{corrplot}\NormalTok{(cor\_matrix, }
     \AttributeTok{method =} \StringTok{"number"}\NormalTok{, }\CommentTok{\# Display numbers}
     \AttributeTok{col =} \FunctionTok{colorRampPalette}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{"blue"}\NormalTok{, }\StringTok{"white"}\NormalTok{, }\StringTok{"red"}\NormalTok{))(}\DecValTok{200}\NormalTok{), }\CommentTok{\# Color gradient}
     \AttributeTok{tl.col =} \StringTok{"black"}\NormalTok{, }\CommentTok{\# Text color for labels}
     \AttributeTok{tl.srt =} \DecValTok{45}\NormalTok{,    }\CommentTok{\# Rotate text labels}
     \AttributeTok{title =} \StringTok{"Correlation Matrix of Selected Variables"}\NormalTok{,}
     \AttributeTok{mar =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{0}\NormalTok{)) }\CommentTok{\# Adjust margins}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-31-1.pdf}}

We can also include circles to graphically represent correlations.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Generate the correlation plot with circles}
\FunctionTok{corrplot}\NormalTok{(cor\_matrix, }
     \AttributeTok{method =} \StringTok{"circle"}\NormalTok{, }\CommentTok{\# Display circles}
     \AttributeTok{col =} \FunctionTok{colorRampPalette}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{"blue"}\NormalTok{, }\StringTok{"white"}\NormalTok{, }\StringTok{"red"}\NormalTok{))(}\DecValTok{200}\NormalTok{), }\CommentTok{\# Color gradient}
     \AttributeTok{tl.col =} \StringTok{"black"}\NormalTok{, }\CommentTok{\# Text color for labels}
     \AttributeTok{tl.srt =} \DecValTok{45}\NormalTok{,    }\CommentTok{\# Rotate text labels}
     \AttributeTok{title =} \StringTok{"Correlation Matrix of Selected Variables"}\NormalTok{,}
...
\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-32-1.pdf}}

\textbf{3+ Numerical Variables: Scatterplot matrix}
...
\NormalTok{selected\_vars }\OtherTok{\textless{}{-}}\NormalTok{ census\_data[, }\FunctionTok{c}\NormalTok{(}\StringTok{"pct\_Males"}\NormalTok{, }\StringTok{"pct\_Single"}\NormalTok{, }\StringTok{"pct\_Bad\_health"}\NormalTok{)]}

\CommentTok{\# Create pairwise combinations of variable names}
\NormalTok{scatter\_data }\OtherTok{\textless{}{-}} \FunctionTok{expand.grid}\NormalTok{(}
 \AttributeTok{Variable1 =} \FunctionTok{names}\NormalTok{(selected\_vars),}
 \AttributeTok{Variable2 =} \FunctionTok{names}\NormalTok{(selected\_vars)}
\NormalTok{)}

\CommentTok{\# Add the data values for the pairs}
\NormalTok{scatter\_data }\OtherTok{\textless{}{-}}\NormalTok{ scatter\_data }\SpecialCharTok{\%\textgreater{}\%}
 \FunctionTok{rowwise}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}
 \FunctionTok{mutate}\NormalTok{(}
  \AttributeTok{Value1 =} \FunctionTok{list}\NormalTok{(selected\_vars[[Variable1]]), }\CommentTok{\# Extract values for Variable1}
  \AttributeTok{Value2 =} \FunctionTok{list}\NormalTok{(selected\_vars[[Variable2]]) }\CommentTok{\# Extract values for Variable2}
\NormalTok{ ) }\SpecialCharTok{\%\textgreater{}\%}
 \FunctionTok{unnest}\NormalTok{(}\FunctionTok{c}\NormalTok{(Value1, Value2)) }\CommentTok{\# Unnest the lists into rows}
\end{Highlighting}
\end{Shaded}

This code is used to create a dataset that facilitates generating
pairwise scatterplots between selected variables from a dataset. First,
it selects the variables of interest (\texttt{pct\_Males},
\texttt{pct\_Single}, and \texttt{pct\_Bad\_health}) from the
census\_data dataset and stores them in a new data frame called
\texttt{selected\_vars}. Then, the \texttt{expand.grid()} function is
used to generate all possible combinations of these variables, resulting
in a data frame with two columns, \texttt{Variable1} and
\texttt{Variable}2, where each row represents a pair of variables (e.g.,
\texttt{pct\_Males} vs \texttt{pct\_Single}). Finally, the
\texttt{mutate()} function is applied to map the actual values of the
paired variables (Variable1 and Variable2) to two new columns, Value1
and Value2. These columns contain the corresponding data points for the
variable pair, enabling the creation of scatterplots where Value1 is
plotted against Value2 for each combination.

Visualisation:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(scatter\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Value1, }\AttributeTok{y =}\NormalTok{ Value2)) }\SpecialCharTok{+}
 ...
 \FunctionTok{facet\_grid}\NormalTok{(Variable1 }\SpecialCharTok{\textasciitilde{}}\NormalTok{ Variable2, }\AttributeTok{scales =} \StringTok{"free"}\NormalTok{) }\SpecialCharTok{+}
 \FunctionTok{labs}\NormalTok{(}
...
\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-34-1.pdf}}

You can also include a categorical variable through colouring. Let's try
with \texttt{Country}.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Ensure the original data includes the \textquotesingle{}Country\textquotesingle{} variable}
\NormalTok{selected\_vars }\OtherTok{\textless{}{-}}\NormalTok{ census\_data[, }\FunctionTok{c}\NormalTok{(}\StringTok{"pct\_Males"}\NormalTok{, }\StringTok{"pct\_Single"}\NormalTok{, }\StringTok{"pct\_Bad\_health"}\NormalTok{, }\StringTok{"Country"}\NormalTok{)]}

\CommentTok{\# Create pairwise combinations of variable names (excluding \textquotesingle{}Country\textquotesingle{})}
\NormalTok{scatter\_data }\OtherTok{\textless{}{-}} \FunctionTok{expand.grid}\NormalTok{(}
  \AttributeTok{Variable1 =} \FunctionTok{names}\NormalTok{(selected\_vars)[}\SpecialCharTok{{-}}\DecValTok{4}\NormalTok{], }\CommentTok{\# Exclude \textquotesingle{}Country\textquotesingle{}}
  \AttributeTok{Variable2 =} \FunctionTok{names}\NormalTok{(selected\_vars)[}\SpecialCharTok{{-}}\DecValTok{4}\NormalTok{]}
\NormalTok{) }
\CommentTok{\#  \%\textgreater{}\% filter(Variable1 != Variable2) \# Remove self{-}comparisons}
...
\NormalTok{scatter\_data }\OtherTok{\textless{}{-}}\NormalTok{ scatter\_data }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{mutate}\NormalTok{(}
    \AttributeTok{Value1 =} \FunctionTok{list}\NormalTok{(selected\_vars[[Variable1]]), }\CommentTok{\# Extract values for Variable1}
    \AttributeTok{Value2 =} \FunctionTok{list}\NormalTok{(selected\_vars[[Variable2]]), }\CommentTok{\# Extract values for Variable2}
    \AttributeTok{Country =} \FunctionTok{list}\NormalTok{(selected\_vars}\SpecialCharTok{$}\NormalTok{Country)     }\CommentTok{\# Include \textquotesingle{}Country\textquotesingle{} in the data}
\NormalTok{  ) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{unnest}\NormalTok{(}\FunctionTok{c}\NormalTok{(Value1, Value2, Country)) }\CommentTok{\# Unnest the lists into rows}
\end{Highlighting}
\end{Shaded}

Now, the scatter\_data includes the \texttt{Country} column, which can
be used to map fill in \texttt{ggplot}:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(scatter\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Value1, }\AttributeTok{y =}\NormalTok{ Value2, }\AttributeTok{fill =}\NormalTok{ Country)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{alpha =} \FloatTok{0.6}\NormalTok{, }\AttributeTok{shape =} \DecValTok{21}\NormalTok{, }\AttributeTok{size =} \DecValTok{3}\NormalTok{) }\SpecialCharTok{+} \CommentTok{\# Use shape with fill (e.g., 21)}
  \FunctionTok{facet\_grid}\NormalTok{(Variable1 }\SpecialCharTok{\textasciitilde{}}\NormalTok{ Variable2, }\AttributeTok{scales =} \StringTok{"free"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{title =} \StringTok{"Scatterplot Matrix with Fill by Country"}\NormalTok{,}
    \AttributeTok{x =} \StringTok{""}\NormalTok{,}
    \AttributeTok{y =} \StringTok{""}\NormalTok{,}
    \AttributeTok{fill =} \StringTok{"Country"}
\NormalTok{  ) }\SpecialCharTok{+}
  \FunctionTok{theme\_minimal}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-36-1.pdf}}

\textbf{1 Numerical Variables, 2+ Cateorical Variables: Boxplot
faceting}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{(frs\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ marital\_status, }\AttributeTok{y =}\NormalTok{ hh\_income\_gross, }\AttributeTok{fill =}\NormalTok{ marital\_status)) }\SpecialCharTok{+}
 \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+}
 \FunctionTok{facet\_wrap}\NormalTok{(}\SpecialCharTok{\textasciitilde{}}\NormalTok{ sex) }\SpecialCharTok{+}
 \FunctionTok{scale\_y\_continuous}\NormalTok{(}
  \AttributeTok{limits =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{100000}\NormalTok{), }\CommentTok{\# limit the y{-}axis}
  \AttributeTok{labels =} \FunctionTok{label\_comma}\NormalTok{() }\CommentTok{\# Properly placed within scale\_y\_continuous()}
\NormalTok{ ) }\SpecialCharTok{+}
 \FunctionTok{labs}\NormalTok{(}
  \AttributeTok{title =} \StringTok{"Gross Household Income by Marital Status and Sex"}\NormalTok{,}
  \AttributeTok{x =} \StringTok{"Marital Status"}\NormalTok{,}
  \AttributeTok{y =} \StringTok{"Gross Household Income"}
\NormalTok{ ) }\SpecialCharTok{+} \CommentTok{\# Removed \textquotesingle{}fill\textquotesingle{} label to avoid legend creation}
 \FunctionTok{theme\_minimal}\NormalTok{() }\SpecialCharTok{+}
 \FunctionTok{theme}\NormalTok{(}
  \AttributeTok{axis.text.x =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{angle =} \DecValTok{45}\NormalTok{, }\AttributeTok{hjust =} \DecValTok{1}\NormalTok{), }\CommentTok{\# Rotate x{-}axis labels for readability}
  \AttributeTok{legend.position =} \StringTok{"none"} \CommentTok{\# Corrected to lowercase \textquotesingle{}none\textquotesingle{}}
\NormalTok{ )}
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-37-1.pdf}}

\textbf{1 Numerical Variables, 2+ Cateorical Variables: Violin Plot
faceting}

A violin plot combines aspects of a box plot and a density plot. It
displays the distribution of a continuous variable, showing its
probability density across different levels of a categorical variable.
The plot consists of a vertical axis representing the variable of
interest and horizontal axes that group data by categorical variables.
The shape of the ``violin'' represents the distribution's density, often
mirrored on both sides, making it easier to see the spread and skewness
of the data.
...
\pandocbounded{\includegraphics[keepaspectratio]{labs/05.Visualisation_files/figure-pdf/unnamed-chunk-38-1.pdf}}

\section{Part 4: Publication-Ready
Tables}\label{part-4-publication-ready-tables}

We will be using \texttt{KableExtra} to create nicely formatted tables
for a series of ouputs in Rstudio:
...
\subsection{Summarising datasets}\label{summarising-datasets}

Only for summarising statistics we use \texttt{vtable} that makes use of
\texttt{kableExtra} behind the scenes. You can use the function
\texttt{st()} or \texttt{sumtable()}, they do the same thing. See
\url{https://cran.r-project.org/web/packages/vtable/vignettes/sumtable.html}
...
\begin{verbatim}
Warning: package 'vtable' was built under R version 4.5.3
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Generate the summary table}
\FunctionTok{st}\NormalTok{(census\_data, }\AttributeTok{vars =} \FunctionTok{c}\NormalTok{(}\StringTok{"pct\_Very\_bad\_health"}\NormalTok{, }\StringTok{"pct\_No\_qualifications"}\NormalTok{, }\StringTok{"pct\_Males"}\NormalTok{, }\StringTok{"pct\_Higher\_manager\_prof"}\NormalTok{), }\AttributeTok{out =} \StringTok{\textquotesingle{}kable\textquotesingle{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{table}

\caption{\label{tab:unnamed-chunk-40}Summary Statistics}
\centering
\begin{tabular}[t]{llllllll}
\toprule
Variable & N & Mean & Std. Dev. & Min & Pctl. 25 & Pctl. 75 & Max\\
\midrule
pct\_Very\_bad\_health & 331 & 1.2 & 0.34 & 0.55 & 0.92 & 1.4 & 2.4\\
pct\_No\_qualifications & 331 & 18 & 4 & 6.6 & 15 & 20 & 29\\
pct\_Males & 331 & 49 & 0.66 & 47 & 49 & 49 & 55\\
pct\_Higher\_manager\_prof & 331 & 13 & 4.7 & 5.5 & 9.8 & 16 & 40\\
\bottomrule
\end{tabular}
\end{table}

\subsection{Creating a Well-Formatted Table from a Cross
Tabulation}\label{creating-a-well-formatted-table-from-a-cross-tabulation}
...
\NormalTok{cross\_tab\_df }\SpecialCharTok{\%\textgreater{}\%}
 \FunctionTok{kbl}\NormalTok{(}\AttributeTok{caption =} \StringTok{"Health and Educational Qualification"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
 ...
 \FunctionTok{add\_header\_above}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{" "} \OtherTok{=} \DecValTok{1}\NormalTok{, }\StringTok{"Highest Educational Qualification"} \OtherTok{=} \DecValTok{2}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\begin{table}

\caption{\label{tab:unnamed-chunk-42}Health and Educational Qualification}
\begin{tabular}[t]{l|l|r}
\hline
\multicolumn{1}{c|}{ } & \multicolumn{2}{c}{Highest Educational Qualification} \\
\cline{2-3}
Health & Qualification & Percentage\\
\hline
...
\item
  ...
  \texttt{colnames(cross\_tab\_df)\ \textless{}-\ c("Health",\ "Qualification",\ "Percentage")}.
  This assigns new column names to the data frame
  \texttt{cross\_tab\_df}.
\item
  \textbf{Generate a Table with \texttt{kable}}:
  \texttt{cross\_tab\_df\ \%\textgreater{}\%\ kbl(caption\ =\ "Health\ and\ Educational\ Qualification")}.
  This uses the \texttt{kbl} function from the \texttt{kableExtra}
  package to create a basic, well-formatted table. The \texttt{caption}
  argument adds a title to the table: ``Health and Educational
  Qualification.
\item
  \textbf{Style the Table}:
  \texttt{\%\textgreater{}\%\ kable\_styling(full\_width\ =\ FALSE,\ position\ =\ "left")}.
  This enhances the appearance of the table with the
  \texttt{kable\_styling} function. \texttt{full\_width\ =\ FALSE}
  ensures the table is not stretched to the full width of the document
  or webpage; \texttt{position\ =\ "left"}: Aligns the table to the left
  side of the page.
\item
  \textbf{Add a Header Row}:
  \texttt{\%\textgreater{}\%\ add\_header\_above(c("\ "\ =\ 1,\ "Highest\ Educational\ Qualification"\ =\ 2))}
  Adds an additional header row above the table; \texttt{"\ "\ =\ 1} the
  first column (``Health'') has a blank header in this new row;
  \texttt{"Highest\ Educational\ Qualification"\ =\ 2}: The next two
  columns (``Qualification'' and ``Percentage'') are grouped under the
  label ``Highest Educational Qualification.
\end{itemize}

\subsection{Creating a Well-Formatted Table from a Cross
Tabulation}\label{creating-a-well-formatted-table-from-a-cross-tabulation-1}

Finally, we can derive a table to show the Multiple Linear Regression
results.
...
\NormalTok{model }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(pct\_Very\_bad\_health }\SpecialCharTok{\textasciitilde{}}\NormalTok{ pct\_No\_qualifications }\SpecialCharTok{+}\NormalTok{ pct\_Males }\SpecialCharTok{+}\NormalTok{ pct\_Higher\_manager\_prof, }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{regression\_table }\OtherTok{\textless{}{-}} \FunctionTok{tidy}\NormalTok{(model) }\SpecialCharTok{\%\textgreater{}\%}
 ...
 \FunctionTok{rename}\NormalTok{(}
  ...
  \StringTok{\textasciigrave{}}\AttributeTok{t value}\StringTok{\textasciigrave{}} \OtherTok{=}\NormalTok{ statistic,}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{regression\_table }\SpecialCharTok{\%\textgreater{}\%}
 \FunctionTok{kbl}\NormalTok{(}
  \AttributeTok{caption =} \StringTok{"Regression Results: Predicting Very Bad Health Percentage"}\NormalTok{,}
...
\NormalTok{ ) }\SpecialCharTok{\%\textgreater{}\%}
 \FunctionTok{kable\_styling}\NormalTok{(}\AttributeTok{full\_width =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{bootstrap\_options =} \FunctionTok{c}\NormalTok{(}\StringTok{"striped"}\NormalTok{, }\StringTok{"hover"}\NormalTok{, }\StringTok{"condensed"}\NormalTok{)) }\SpecialCharTok{\%\textgreater{}\%}
 \FunctionTok{column\_spec}\NormalTok{(}\DecValTok{2}\SpecialCharTok{:}\DecValTok{5}\NormalTok{, }\AttributeTok{width =} \StringTok{"3cm"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\# Adjust column widths}
 \FunctionTok{add\_header\_above}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{" "} \OtherTok{=} \DecValTok{1}\NormalTok{, }\StringTok{"Coefficients"} \OtherTok{=} \DecValTok{4}\NormalTok{)) }\CommentTok{\# Add grouped header}
\end{Highlighting}
\end{Shaded}

\begin{table}
\centering
\caption{\label{tab:unnamed-chunk-45}Regression Results: Predicting Very Bad Health Percentage}
\centering
\begin{tabular}[t]{l|>{\raggedleft\arraybackslash}p{3cm}|>{\raggedleft\arraybackslash}p{3cm}|>{\raggedleft\arraybackslash}p{3cm}|>{\raggedleft\arraybackslash}p{3cm}}
\hline
\multicolumn{1}{c|}{ } & \multicolumn{4}{c}{Coefficients} \\
\cline{2-5}
Term & Estimate & Std. Error & t Value & P Value\\
\hline
(Intercept) & 4.003 & 0.880 & 4.550 & 0.000\\
\hline
pct\_No\_qualifications & 0.053 & 0.006 & 8.937 & 0.000\\
\hline
pct\_Males & -0.074 & 0.018 & -4.121 & 0.000\\
\hline
pct\_Higher\_manager\_prof & -0.013 & 0.005 & -2.670 & 0.008\\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
\item
  \texttt{kable\_styling()} This function customizes the appearance of a
  \textbf{\texttt{kable}} table. The arguments used are:

  \begin{itemize}
  \item
    \texttt{full\_width\ =\ FALSE}: Ensures the table is not stretched
    to fill the entire width of the page.
  \item
    \texttt{bootstrap\_options\ =\ c("striped",\ "hover",\ "condensed")}:

    \begin{itemize}
    \item
      \texttt{striped}: Adds alternating row colors for better
      readability.
    \item
      \texttt{hover}: Highlights rows when hovered over with the mouse
      (for HTML tables).
    \item
      \texttt{condensed}: Reduces the table's vertical spacing, making
      it more compact.
    \end{itemize}
  \end{itemize}
\item
  \texttt{column\_spec(2:5,\ width\ =\ "3cm")}. This function customizes
  specific columns of the table.

  \begin{itemize}
  \item
    \texttt{2:5}: Targets columns 2 through 5 for styling.
  \item
    \texttt{width\ =\ "3cm"}: Sets the width of these columns to 3 cm,
    ensuring uniform and readable column sizes.
  \end{itemize}
\item
  \texttt{add\_header\_above(c("\ "\ =\ 1,\ "Coefficients"\ =\ 4))}.
  This function adds an additional header row above the table. This is
  used to customize the header of a table.

  \begin{itemize}
  \tightlist
  \item
    \texttt{"\ "\ =\ 1} Leaves the first column (e.g., the Term column)
    without a header, spanning 1 column.
  \item
    \texttt{"Coefficients"\ =\ 4}: This part means that columns 2, 3, 4,
    and 5 will be grouped together under one header, which is labeled as
    ``Coefficients'' (4 here indicates the other 4 tables). Essentially,
    these columns will have the same header, which makes the table more
    readable and organized.
  \end{itemize}
\end{itemize}

Have a look here for some details on \texttt{kableExtra}
\url{https://bookdown.org/yihui/rmarkdown-cookbook/kableextra.html}.

\section{Part 5: Play with the code}\label{part-5-play-with-the-code}

\begin{itemize}
\tightlist
\item
  Experiment with different variables and aesthetics to deepen your
  understanding of ggplot2.
\item
  Test modifying labels, themes, and colour schemes to create tailored
  visualisations.
\end{itemize}
