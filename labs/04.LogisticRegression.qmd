...
We have learnt that for the categorical/qualitative variables, we need R to treat them as `factor` type. Therefore, if we want to create descriptive analysis charts by using `sar_code` , we need first convert the code numbers from numeric to factor type. But it may not that readable as a result chart put in the report as the Y-axis are all in codes rather than labels.
...
Based on 'SAR_dictionary.xlsx', we learn how the codes map to the categories for the variable work_distance in both csv.
...
::: {style="background-color: #FFFBCC; padding: 10px; border-radius: 5px; border: 1px solid #E1C948;"}
**Q2**. **Can you write down the findings you learnt from the model outcomes to other occupation groups (nssec = 4, nssec = 6 and nssec =7)?**
:::
...
### Recode Socio-economic status variable and explore
...
::: {style="background-color: #FFFBCC; padding: 10px; border-radius: 5px; border: 1px solid #E1C948;"}
**Q3**. If we want to explore whether people work as independent employers show lower probability of commuting longer than 60km compared with other occupations, how will we prepare the input independent variables and what will be the specified regression model?
:::
...
::: {style="background-color: #FFFBCC; padding: 10px; border-radius: 5px; border: 1px solid #E1C948;"}
**Q4.** **Now what conclusion you can draw when comparing the lower NSSEC groups to the higher groups?**
:::
